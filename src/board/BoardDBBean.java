...
public class BoardDBBean {
    ...
    throws Exception {
        ...
        try {
		    ...
		    sql+="ref,re_step,re_level,content,readcount) values(seq_board_id.NEXTVAL,?,?,?,?,?,?,?,?,?,?)";
//readconunt 추가
            pstmt = conn.prepareStatement(sql);
...
//------------------------------
    ...
    throws Exception {
        ...
        try {
            ...
            pstmt = conn.prepareStatement("select count(writer) from board where " + gubun + " like ?");
...
//            	"select * from board order by ref desc, re_step asc limit ?,? ");
 //           pstmt = conn.prepareStatement("select * from (select * from board order by ref desc,re_step asc) board from rownum > ? and rownum <=?");
             pstmt = conn.prepareStatement("select * from (select board.* , rownum as rnum from(select * from  board order by ref desc, re_step asc) board) where rnum > ?  and rnum <= ? ");
    ...
    throws Exception {
        ...
        try {
            ...
            pstmt = conn.prepareStatement("select * from (select board.* , rownum as rnum from(select * from board where " + gubun + " like ?  order by ref desc, re_step asc) board) where rnum > ?  and rnum <= ? ");
